...
export function ResyApi() {
    ...
    const annotations = {
        getTags() {
            return RequestBuilder.get({
                baseRequestUrl: ServicesConfig.getService("annotationService").baseRequestUrl,
                endpoint: "1/tags",
            });
        },
    };
    const geoip = () => RequestBuilder.get({
        endpoint: "3/geoip",
    });
    const items = {
        getByVenue(venueId, data) {
            return RequestBuilder.get({
                baseRequestUrl: ServicesConfig.getService("itemsService").baseRequestUrl,
    ...
    const contact = (userInfo) => RequestBuilder.post({
        endpoint: "2/contact",
        data: {
            value: userInfo,
        },
    });
    const find = (data) => RequestBuilder.get({
        endpoint: "4/find",
        data,
    }).then((response) => ResyTransformer.finder4(response));
    const getSignedMapUrl = (url) => RequestBuilder.get({
        endpoint: "3/google/static_maps/url/sign",
        data: {
            url,
        },
    });
    const topVenues = (data) => RequestBuilder.get({
    ...
    const hospitality = (data) => RequestBuilder.post({
    ...
    const rating = {
        submit(data) {
            ...
            if ("uniqueKey" in data && !!data.uniqueKey) {
                ...
                console.error("[ERROR] ResyApi /3/rate/reservation is missing a required parameter.");
                // eslint-disable-line
                return;
            }
            if ("comments" in data &&
    ...
    const party = {
        ...
        getTags(partyId) {
            return RequestBuilder.get({
                baseRequestUrl: ServicesConfig.getService("resyosService").baseRequestUrl,
    ...
    const listVenues = (locationId) => RequestBuilder.get({
        endpoint: "2/venues",
        data: {
            location_id: locationId,
        },
    }).then((response) => ResyTransformer.listVenues(response));
    const location = (lat, long) => RequestBuilder.get({
        endpoint: "3/location",
        data: {
            lat,
            long,
    ...
    const locations = (lat, long) => RequestBuilder.get({
    ...
    const user = {
        ...
        getReservation(resyToken) {
            const bookOnBehalfOf = arguments.length > 1 && arguments[1] !== undefined
                ? arguments[1]
                : false;
            let endpoint = "3/user/reservations";
            const data = {};
    ...
    const statistics = () => RequestBuilder.get({
    ...
    const amexOffers = () => RequestBuilder.get({
