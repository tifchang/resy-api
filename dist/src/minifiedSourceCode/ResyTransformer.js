...
import { HIT_SCHEMA, NOTIFY_SCHEMA, RESERVATION_SCHEMA, SLOT_SCHEMA, VENUE_SCHEMA, } from "./schemas";
function ResyModifiers() {
    ...
    function getWhyWeLikeIt(data) {
        const whyWeLikeIt = data.find((content) => content.name === "why_we_like_it");
...
function autocompleteSearch(source) {
    const { search: { hits } /* eslint-disable-next-line no-shadow */, suggestions, } = source;
    const res = {
...
function search(source) {
    ...
    return {
        results: [
            {
                hits: hits.map((hit) => {
                    const transformedSearchResult = transform(hit, HIT_SCHEMA.SEARCH_RESULT);
                    const activeNotificationsArray = Object.keys(transformedSearchResult.activeNotifications).map((serviceTypeId) => ({
                        [serviceTypeId]: transform(transformedSearchResult.activeNotifications[serviceTypeId], NOTIFY_SCHEMA),
                    }));
                    const activeNotifications = activeNotificationsArray.reduce((memo, notifies) => {
                    ...
                    return {
                        ...
                        activeNotifications,
                        slots: hit.availability.slots.map((slot) => transform(slot, SLOT_SCHEMA.SEARCH.FLY)),
                    };
                }),
                nbPages,
            },
...
function transform(source, schema, response) {
    ...
    Object.keys(schema).forEach((key) => {
        if (Utils.isArray(schema[key])) {
            localResponse[key] = source[key].map((element) => transform({
                ...element,
                ...source,
            }, schema[key][0]));
        }
